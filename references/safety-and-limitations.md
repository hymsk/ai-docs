...
Markmap 原始 HTML、危险路径和配置边界仍会被拒绝，不应通过非严格模式绕过。

`dot`/`graphviz` fence 不受支持，并且不会被 `--no-strict` 降级为普通代码块或错误卡片。构建器会明确失败，避免文档在迁移后静默改变语义；请改用 Mermaid flowchart/subgraph。
